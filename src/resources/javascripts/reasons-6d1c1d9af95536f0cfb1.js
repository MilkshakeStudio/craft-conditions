! function (e) {
	...
	! function (e) {
		/*** action */
		return !!e.Craft && void(Craft.ConditionsPlugin = {
			...
			initPrimaryForm: function () {
				this.destroyPrimaryForm(), Garnish.requestAnimationFrame(function () {
				...
				var i = (new Date).getTime(),
					s = function () {
						var e = (new Date).getTime(),
							n = $(".elementeditor:last"),
							a = n.length > 0 && n.closest(".hud"),
							r = !!(a && a.length > 0) && a.data("elementEditor"),
							o = !!r && r.$form;
						o ? (r._reasonsForm = new this.ConditionalsRenderer(o, t), r.hud.on("hide", $.proxy(this.destroyElementEditorForm, this, r))) : e - i < 2e3 && Garnish.requestAnimationFrame(s)
					}.bind(this);
				s()
			},
			destroyElementEditorForm: function (e) {
				var t = e._reasonsForm || null;
				t && (t.destroy(), delete e._reasonsForm)
			},
			initForm: function (e) {
			...
			getConditionals: function (e) {
				//alert('qqqqqxx');
			console.log(e);	
			//	alert('getConditionals');
			//	console.log('xxxx');
			//	console.log(e);
			//	console.log(JSON.parse(this.data.conditionals[e]));
				return e ? this.data.conditionals && this.data.conditionals.hasOwnProperty(e) ? JSON.parse(this.data.conditionals[e]): null : this.data.conditionals || {}
			},
			...
			getToggleFieldById: function (e) {
				e = parseInt(e);
				for (var t = this.getToggleFields(), i = t.length, s = 0; s < i; ++s)
					if (parseInt(t[s].id) === e) return t[s];
				return !1
			},
			...
			getElementSourceFromForm: function (e) {
				...
				t && (t += "-");
				var i, s, n = e.find('input[type="hidden"][name="action"]').val();
				...
				switch (n) {
					...
					case this.ENTRY_REVISION_ACTION:
						...
						i = a.length ? this.ENTRY_TYPE_HANDLE : this.SECTION_HANDLE, s = a.length ? 'select#entryType, input[type="hidden"][name="entryTypeId"], input[type="hidden"][name="typeId"], #' + t + "entryType" : 'input[type="hidden"][name="sectionId"], #' + t + "section";
						break;
 ...
 function (e, t, i) {
	...
	e.exports = function () {
		...
		function e(t, i) {
			...
			s(this, e), t && t.length && (this.$el = t, this.conditionals = i, this.settings = {
		...
		return n(e, [{
			...
			value: function () {
				...
				this.toggleFieldIds = $.map(e, function (e) {
					...
					value: this.id || "",
					type: "hidden"
				})), this.$el.append(this.$conditionalsInput).append(this.$conditionalsIdInput).on("submit", $.proxy(this.onFormSubmit, this)), Garnish.requestAnimationFrame($.proxy(function () {
					this.refresh()
				}, this)), this.$el.on("mousedown", this.settings.fieldSelector, $.proxy(this.onFieldMouseDown, this)), Garnish.$doc.on("click", ".menu a", $.proxy(this.onFieldSettingsMenuItemClick, this))
			}
		}, {
			key: "destroy",
			value: function () {
			...
			value: function () {
				...
				this.$el.find(this.settings.tabSelector).each(function () {
					...
					var s = [];
					 ...
					 $fields.each(function () {
						t = $(this), i = parseInt(t.data("id")), t.data("_reasonsBuilder") ? t.data("_reasonsBuilder").update({
							toggleFields: s
						}) : t.data("_reasonsBuilder", new a({
							fieldId: i,
							toggleFields: s,
							rules: n.conditionals && n.conditionals.hasOwnProperty(i) ? n.conditionals[i] : null
						}));
						 
						var e = t.data("_reasonsBuilder").getConditionals();
						if (e ? (r[i] = e, t.addClass("reasonsHasConditionals")) : t.removeClass("reasonsHasConditionals"), !t.data("_reasonsSettingsMenuItemInitialized")) {
							var o = t.find(n.settings.fieldSettingsSelector),
								l = o.data("menubtn") || !1;
							if (!l) return;
							var d = l.menu.$container;
							
							d.find("ul").children(":first").clone(!0).insertBefore(d.find("ul:first li:last")).find("a:first").data("_reasonsField", t).attr("data-action", "toggle-conditionals").html("Edit conditionals"), t.data("_reasonsSettingsMenuItemInitialized", !0);
							d.find("ul").children().eq(1).find("a:first").html("Edit conditionals");
						}
					})
			...
			value: function (e) {
				var t = this,
					i = function e(i) {
						$("body").off("mouseup", e), Garnish.requestAnimationFrame(function () {
							t.refresh()
						})
					};
				$("body").on("mouseup", i)
	...
	var i = Garnish.Base.extend({
		...
		init: function (e) {
			...
			this.$container.on("click", ".reasonsAddRule", $.proxy(this.onReasonsAddRuleClick, this))
		...
		refresh: function () {
			...
			var t, i, s, n, a, r, o, l = this,
				d = this.$container.find(".reasonsStatement");
			d.each(function () {
				return t = [], i = $(this), s = i.find(".reasonsRule"), s.each(function () {
					n = $(this);
					
					if (a = n.find(".reasonsRuleToggleField select"), r = parseInt(a.val()), l.settings.toggleFieldIds.indexOf(r) === -1) return void n.remove();
					o = "";
			...
			//console.log(n);
					t.push({
						fieldId: r,
						compare: n.find(".reasonsRuleCompare select").val(),
						value: n.find(".reasonsRuleValue *:input:first").val()
					})
		...
		addStatement: function (e) {
			...
			if (this.$builder.append(t), i)
				for (var s = 0; s < i.length; ++s) this.addRule($.extend({
					target: t
				}, i[s]));
			else this.addRule({
				target: t
		...
		addRule: function (e) {
			...
			var t = this.$rule.clone(!0),
				i = e.target || this.$builder.find(".reasonsStatement:last"),
				s = e.fieldId,
				n = e.compare,
				a = e.value;
			if(s)
			{
				t.find(".reasonsRuleToggleField select").val(s);
			}
			if(i.length > 0)
			{
				i.find(".reasonsRules:first").append(t);
			}
			else {
				return false;
			}
			t.find(".reasonsRuleToggleField select").trigger("change");
			if(n)
			{
				 t.find(".reasonsRuleCompare select").val(n);
			}
			if(a)
			{
				  t.find(".reasonsRuleValue *:input:first").val(a);
			}
			/*
			alert('pppp');
			console.log(e);
		...
		onReasonsAddRuleClick: function (e) {
			e.preventDefault();
			var t = $(e.currentTarget).parents(".reasonsStatement");
			this.addRule({
				target: t
			})
		},
		...
		onReasonsRuleToggleFieldChange: function (e) {
			...
			var t = $(e.currentTarget),
				...
				r = a.type,
				o = a.settings,
				superTableColumns =  i.find(".reasonsRuleSuperTableColumns"),
				l = "";
				...
				*/
				...
				input: function e(t) {
					...
					return t = $.extend({
						...
						placeholder: "",
						multiline: !1
					}, t), e += "1" === t.multiline ? '<textarea rows="' + t.initialRows + '" placeholder="' + t.placeholder + '" autocomplete="off"></textarea>' : '<input class="text" type="text" size="20" value="" placeholder="' + t.placeholder + '" autocomplete="off">', '<div class="input">' + e + "</div>"
				},
				builderUi: function () {
                                            ...
                                            '<div class="reasonsRule">' +
                                                '<div class="reasonsRuleParams">' +
                                                    '<div class="select reasonsRuleToggleField"><select /></div>' +
						  							'<div class=" reasonsRuleSuperTableColumns"></div>' +
                                                ...
                                                '<div class="reasonsRuleAmend">' +
                                                    '<a class="delete icon reasonsRemoveRule" title="Remove rule"></a>' +
                                                    '<a class="add icon reasonsAddRule" title="and"></a>' +
                                                '</div>' +
                                                '<span class="reasonsRuleLead">and</span>' +
                                            '</div>' +
                                        '</div>' +
                                    '</div>' +
                                '</div>' +
                                '<div class="reasonsAdd">' +
                                    '<a class="btn reasonsAddStatement">Add rules</a>' +
                                '</div>' +
                            '</div>' +
	...
	e.exports = function () {
		...
		*/
		...
		function e(t, s) {
			...
			}, this.$el = t, this.id = this.$el.attr("id"), this.id || (this.id = "_reasonsForm-" + Math.random().toString(36).slice(2), this.$el.attr("id", this.id)), this.conditionals = s, $(this.init.bind(this)), this
		}
		return s(e, [{
			key: "init",
			value: function () {
		//	alert('aaaaa');
			...
			value: function () {
				...
				Garnish.$doc.on("click", this.settings.fieldsSelector + '[data-toggle="1"]', this.onInputWrapperClick.bind(this)).on("change keyup", this.settings.fieldsSelector + '[data-toggle="1"] *:input, ' +this.settings.fieldsSelector + ' *:input, '+ this.settings.fieldsSelector + '[data-toggle="1"] ' + this.settings.lightswitchContainerSelector, this.onFieldInputChange.bind(this)).on("click", "a[data-buttonbox-value]", this.onFieldInputChange.bind(this));
				for (var e, t = this, i = ["elementselect", "categoriesfield"], s = 0; s < i.length; ++s) $(this.settings.fieldsSelector + " ." + i[s]).each(function () {
					if (!$(this).hasAttr("data-reasonselementselect")) {
						var i = (new Date).getTime(),
							s = function () {
								e = $(this).data("elementSelect"), e ? (e.on("selectElements", t.onElementSelectChange.bind(t)), e.on("removeElements", t.onElementSelectChange.bind(t)), $(this).attr("data-reasonselementselect", ""), t.onElementSelectChange()) : (new Date).getTime() - i < 2e3 && Garnish.requestAnimationFrame(s)
							}.bind(this);
						s()
					}
				})
			...
			value: function () {
				Garnish.$doc.off("click", this.settings.fieldsSelector + '[data-toggle="1"]', this.onInputWrapperClick.bind(this)).off("change keyup", this.settings.fieldsSelector + '[data-toggle="1"] *:input, ' + this.settings.fieldsSelector + '[data-toggle="1"] ' + this.settings.lightswitchContainerSelector, this.onFieldInputChange.bind(this)).off("click", "a[data-buttonbox-value]", this.onFieldInputChange.bind(this));
				var e, t = this;
				$("[data-reasonselementselect]").each(function () {
					e = $(this).data("elementSelect"), e && (e.off("selectElements", t.onElementSelectChange.bind(t)), e.off("removeElements", t.onElementSelectChange.bind(t))), $(this).removeAttr("[data-reasonselementselect]")
				})
			}
		}, {
			...
			value: function () {
				function e() {
					Craft.livePreview ? (this._livePreview = Craft.livePreview, this._livePreview.on("enter", $.proxy(this.onLivePreviewEnter, this)), this._livePreview.on("exit", $.proxy(this.onLivePreviewExit, this)), this._livePreviewPollId && delete this._livePreviewPollId) : (new Date).getTime() - i < 2e3 && (this._livePreviewPollId = Garnish.requestAnimationFrame(t))
				}
				var t = e.bind(this),
					i = (new Date).getTime();
				t()
			...
			value: function () {
				this._livePreviewPollId && (Garnish.cancelAnimationFrame(this._livePreviewPollId), delete this._livePreviewPollId), this._livePreview && (this._livePreview.off("enter", $.proxy(this.onLivePreviewEnter, this)), this._livePreview.off("exit", $.proxy(this.onLivePreviewExit, this)), delete this._livePreview)
			}
		}, {
			key: "destroy",
			value: function () {
			...
			value: function () {
				...
				var a, r, o, l, d = this;
				return this.$fields.each(function () {
					a = $(this), void 0 !== a.attr("id") && (r = a.attr("id").split("-"), r.length < 3 || r.length > 4 || (o = r.slice(-2, -1)[0] || !1, o && (l = Craft.ConditionsPlugin.getFieldIdByHandle(o), l && a.attr("data-id", l), d.conditionals[l] && a.attr("data-target", 1), i.indexOf(parseInt(l)) > -1 && a.attr("data-toggle", 1))))
				}), !0
			}
		}, {
			key: "evaluateConditionals",
			value: function () {
				var e, t, i, s, n, a, r, o, l, d = this,
					u = $(this.getFieldsSelector() + '[data-target="1"]');
				u.removeClass("reasonsHide").removeAttr("aria-hidden").removeAttr("tabindex").each(function () {
					e = $(this);
					...
					if ( t) {
							...
							//	console.log(rule);
								...
								//	console.log(o);
									...
									switch (o.type) {
										...
										case "ButtonBox_Stars":
										case "ButtonBox_Width":
											l = a.find("input:checkbox:checked,input:radio:checked").map(function () {
												return $(this).val()
											}).get();
										...
										case "Users":
										case "Calendar_Event":
											var g = a.find("[data-reasonselementselect]").data("elementSelect") || null;
						...
						console.log(h);
						console.log(e);
						h <= 0 && e.addClass("reasonsHide").attr("aria-hidden", "true").attr("tabindex", "-1")
					}
				})
			}
		}, {
